...
from .utils import q, q_safe, col_exists, latex_escape, fmt, NOT_FP, ACCEPTED, SHOW_PER, DB_PATH, cookie_party
...
def run(conn):
    ...
    post_tracker_rate = post_trackers_total / post_requests_total * 100 if post_requests_total and has_tracker_col else 0
    ...
    if has_reject_col:
        rej_rows = q_safe(conn, f"SELECT url FROM chrome_scans WHERE cookie_notice_rejected=1 AND is_error_page=0 AND {NOT_FP}")
        if rej_rows:
            rejected_urls = {r[0] for r in rej_rows}

    notice_urls = set()
    notice_rows = q_safe(conn, f"SELECT url FROM chrome_scans WHERE cookie_notice_detected=1 AND is_error_page=0 AND {NOT_FP}")
    if notice_rows:
        notice_urls = {r[0] for r in notice_rows}

    sites_with_notice = [(url, pre_t, post_t, rej_t, pre_tot, post_tot, rej_tot)
                         for url, pre_t, post_t, rej_t, pre_tot, post_tot, rej_tot in tracker_rows
                         if url in notice_urls]
    sites_without_notice = [(url, pre_t, post_t, rej_t, pre_tot, post_tot, rej_tot)
                            for url, pre_t, post_t, rej_t, pre_tot, post_tot, rej_tot in tracker_rows
                            if url not in notice_urls]
    sites_with_post = [(url, pre_t, post_t, rej_t, pre_tot, post_tot, rej_tot)
                       for url, pre_t, post_t, rej_t, pre_tot, post_tot, rej_tot in tracker_rows
                       if url in accepted_urls]
    sites_with_reject = [(url, pre_t, post_t, rej_t, pre_tot, post_tot, rej_tot)
                         for url, pre_t, post_t, rej_t, pre_tot, post_tot, rej_tot in tracker_rows
    ...
    avg_no_notice_t   = sum(r[1] or 0 for r in sites_without_notice) / len(sites_without_notice) if sites_without_notice else 0
    ...
    tracker_increased = sum(1 for _, pre_t, post_t, *_ in sites_with_post
                            if post_t is not None and pre_t is not None and post_t > pre_t)
    tracker_decreased = sum(1 for _, pre_t, post_t, *_ in sites_with_post
                            if post_t is not None and pre_t is not None and post_t < pre_t)
    tracker_same_acc  = sum(1 for _, pre_t, post_t, *_ in sites_with_post
                            if post_t is not None and pre_t is not None and post_t == pre_t)

    # post-reject vs pre-accept (baseline)
    rej_vs_pre_more = sum(1 for _, pre_t, _, rej_t, *_ in sites_with_reject
                          if rej_t is not None and pre_t is not None and rej_t > pre_t)
    rej_vs_pre_less = sum(1 for _, pre_t, _, rej_t, *_ in sites_with_reject
                          if rej_t is not None and pre_t is not None and rej_t < pre_t)
    rej_vs_pre_same = sum(1 for _, pre_t, _, rej_t, *_ in sites_with_reject
                          if rej_t is not None and pre_t is not None and rej_t == pre_t)

    # post-reject vs post-accept (for sites that have both)
    sites_with_both = [(url, pre_t, post_t, rej_t, pre_tot, post_tot, rej_tot)
                       for url, pre_t, post_t, rej_t, pre_tot, post_tot, rej_tot in tracker_rows
                       if url in accepted_urls and url in rejected_urls]
    rej_vs_acc_more = sum(1 for _, _, post_t, rej_t, *_ in sites_with_both
                          if rej_t is not None and post_t is not None and rej_t > post_t)
    rej_vs_acc_less = sum(1 for _, _, post_t, rej_t, *_ in sites_with_both
                          if rej_t is not None and post_t is not None and rej_t < post_t)
    rej_vs_acc_same = sum(1 for _, _, post_t, rej_t, *_ in sites_with_both
                          if rej_t is not None and post_t is not None and rej_t == post_t)
    ...
    no_notice_dist = bucket_counts([r[1] or 0 for r in sites_without_notice]) if has_tracker_col and sites_without_notice else None
    ...
    print(
        ...
        rf"sites \emph{{without}} a cookie notice averaged \textbf{{{avg_no_notice_t:.1f}}}. "
    ...
    if sites_with_reject:
        print(rf"Avg.\ tracker requests/site & {avg_pre_t:.1f} & {avg_post_t:.1f} & {avg_reject_t:.1f} \\")
        print(rf"Total tracker requests & {pre_trackers_total} & {post_trackers_total} & {reject_trackers_total} \\")
        print(rf"Total network requests & {pre_requests_total} & {post_requests_total} & {reject_requests_total} \\")
        print(rf"Tracker rate & {pre_tracker_rate:.1f}\,\% & {post_tracker_rate:.1f}\,\% & {reject_tracker_rate:.1f}\,\% \\")
    else:
        print(rf"Avg.\ tracker requests/site & {avg_pre_t:.1f} & {avg_post_t:.1f} \\")
        print(rf"Total tracker requests & {pre_trackers_total} & {post_trackers_total} \\")
        print(rf"Total network requests & {pre_requests_total} & {post_requests_total} \\")
        print(rf"Tracker rate & {pre_tracker_rate:.1f}\,\% & {post_tracker_rate:.1f}\,\% \\")
    ...
    if pre_dist is not None or no_notice_dist is not None:
        ...
        col_fmt = "l" + ("r" if has_no_notice_dist else "") + ("r" if pre_dist is not None else "") + ("r" if has_post_dist else "") + ("r" if has_rej_dist else "")
        hdr_phases = ""
    ...
    # =========================================================
    if has_tracker_col:
        _tp_rows = q_safe(conn, """
            SELECT r.phase, r.site_url, r.request_url, r.is_tracker, r.scan_id
            FROM chrome_network_requests r
            JOIN chrome_scans c ON c.id = r.scan_id
            WHERE c.is_error_page=0 AND r.is_tracker IS NOT NULL
        """)
        ...
        def _tp_n(ph, p, t):  return _tp_counts[(ph, p, t)][0]
        def _tp_s(ph, p, t):  return len(_tp_counts[(ph, p, t)][1])
        ...
        if _tp_pre_tot > 0:
            ...
            if _has_rej_tp:
                ...
                print(r"\toprule Party & \multicolumn{2}{c}{Pre} & \multicolumn{2}{c}{Post-accept} & \multicolumn{2}{c}{Post-reject} \\")
                ...
                print(r"\midrule")
                print(rf"  \textbf{{Total}} & \textbf{{{_tp_pre_tot}}} & & \textbf{{{_tp_acc_tot}}} & & \textbf{{{_tp_rej_tot}}} & \\")
            else:
            ...
            if _has_rej_tp:
                ...
                for _pl, _pk, _rq_pre, _rq_acc, _rq_rej in [
                    ...
                    print(rf"  {_pl} & {_rq_pre} & {_pr:.1f}\,\% & {_rq_acc} & {_ar:.1f}\,\% & {_rq_rej} & {_rr:.1f}\,\% \\")
            else:
                print(r"\begin{tabular}{lrrrr}")
                print(r"\toprule Party & \multicolumn{2}{c}{Pre} & \multicolumn{2}{c}{Post-accept} \\")
                print(r"\cmidrule(lr){2-3}\cmidrule(lr){4-5}")
    ...
    if SHOW_PER:
        if sites_with_reject:
            ...
            for url, pre_t, post_t, rej_t, pre_tot, post_tot, rej_tot in tracker_rows:
                ...
                d_acc = (rf"\textbf{{{post_t - pre_t:+d}}}" if post_t - pre_t != 0 else "0") \
                    if pre_t is not None and post_t is not None else "---"
                d_rej = (rf"\textbf{{{rej_t - pre_t:+d}}}" if rej_t - pre_t != 0 else "0") \
