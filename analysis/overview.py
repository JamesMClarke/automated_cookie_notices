...
from .utils import q, q_safe, col_exists, latex_escape, fmt, NOT_FP, ACCEPTED, DB_PATH
...
def run(conn):
    ...
    manually_verified_count = q(conn,
        f"SELECT COUNT(*) FROM chrome_scans WHERE manually_verified=1 AND {NOT_FP}")[0][0]

    cookie_rejected = 0
    reject_attempted = 0
    if has_reject_cols:
        cookie_rejected  = q(conn, f"SELECT COUNT(*) FROM chrome_scans WHERE cookie_notice_rejected=1 AND {NOT_FP}")[0][0]
        reject_attempted = q(conn, f"SELECT COUNT(*) FROM chrome_scans WHERE cookie_reject_attempted=1 AND {NOT_FP}")[0][0]
    ...
    _top_rising_cat = max(
        ...
        key=lambda c: cat_by_phase.get("post_accept", {}).get(c, 0)
                    - cat_by_phase.get("pre", {}).get(c, 0),
        default=None,
    )
    ...
    if _top_rising_cat and pre_total and acc_total:
        ...
        print(
            ...
            rf"{_acc_pct:.0f}\,\% post-accept"
            + (rf" ({_rej_n} post-reject)" if _rej_n is not None else "")
            + r"."
        )
    print()
    ...
    if has_reject_cols and reject_attempted:
        print(rf"Cookie notice rejected & {cookie_rejected} ({cookie_rejected/reject_attempted*100:.0f}\,\% of attempted) \\")
    ...
    if _top_rising_cat:
        ...
        print(rf"Top rising category & {latex_escape(_top_rising_cat)} ({_pre_pct:.0f}\,\% $\to$ {_acc_pct:.0f}\,\%) \\")
