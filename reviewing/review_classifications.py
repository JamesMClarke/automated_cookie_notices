...
Usage:
    python reviewing/review_classifications.py top-50.db
    python reviewing/review_classifications.py top-50.db --artifacts /nas.local/backups/cookie_notices_automation/chrome
...
_parser.add_argument(
    "--unclassified-only",
    action="store_true",
    help="Only include notices that need manual classification",
)
_parser.add_argument(
    "--control-type",
    default=None,
    help="Filter to sites whose effective control type matches this value "
    "(e.g. accept_only, informational_only). The classify panel is always shown for matched sites.",
)
_args = _parser.parse_args()

DB_PATH = Path(_args.db)
ARTIFACTS_ROOT: Path | None = _args.artifacts
UNCLASSIFIED_ONLY: bool = _args.unclassified_only
CONTROL_TYPE_FILTER: str | None = _args.control_type

con = sqlite3.connect(DB_PATH)
con.row_factory = sqlite3.Row

# Detect which columns exist so we can handle DBs that pre-date the reject phase
_existing_cols = {row[1] for row in con.execute("PRAGMA table_info(chrome_scans)")}


def _col(name, alias=None):
    """Return 'name' if the column exists, else 'NULL AS name' (or alias)."""
    label = alias or name
    return name if name in _existing_cols else f"NULL AS {label}"


_unclassified_where = (
    """
    AND cookie_notice_detected = 1
    AND (false_positive IS NULL OR false_positive != 1)
    AND (
        ((cookie_position IS NULL OR cookie_position = 'none') AND manual_cookie_position IS NULL)
        OR
        ((cookie_emphasized_option IS NULL OR cookie_emphasized_option = 'none') AND manual_cookie_emphasized_option IS NULL)
    )
"""
...
_control_type_where = (
    f" AND COALESCE(manual_cookie_control_type, cookie_control_type) = '{CONTROL_TYPE_FILTER}'"
...
rows = con.execute(f"""
    SELECT id,
           ...
           {_col("cookie_notice_rejected")},
           {_col("cookie_reject_attempted")},
           manually_verified,
           cookie_position,
           cookie_control_type,
           ...
           cookie_bbox_height,
           pre_screenshot_path,
           {_col("post_accept_screenshot_path")},
           {_col("post_reject_screenshot_path")},
           pre_lh_path,
           {_col("post_accept_lh_path")},
           {_col("post_reject_lh_path")},
           pre_lh_score,
           {_col("post_accept_lh_score")},
           {_col("post_reject_lh_score")},
           pre_wave_error,
           pre_wave_contrast,
           pre_wave_alert,
           pre_wave_feature,
           pre_wave_structure,
           pre_wave_aria,
           {_col("post_accept_wave_error")},
           {_col("post_accept_wave_contrast")},
           {_col("post_accept_wave_alert")},
           {_col("post_accept_wave_feature")},
           {_col("post_accept_wave_structure")},
           {_col("post_accept_wave_aria")},
           {_col("post_reject_wave_error")},
           {_col("post_reject_wave_contrast")},
           {_col("post_reject_wave_alert")},
           {_col("post_reject_wave_feature")},
           {_col("post_reject_wave_structure")},
           {_col("post_reject_wave_aria")},
           ...
           false_positive
    FROM chrome_scans
    WHERE 1=1 {_unclassified_where} {_control_type_where}
...
def resolve_path(raw):
    ...
    if ARTIFACTS_ROOT is not None:
        parts = p.parts
        # Find a known crawl/artifacts root component and keep everything after it.
        # Matches 'artifacts' (older scans) or 'crawl_*' (e.g. crawl_two, crawl_three).
        _PIVOT_RE = re.compile(r"^(artifacts|crawl_\w+)$", re.IGNORECASE)
        try:
            idx = next(i for i, part in enumerate(parts) if _PIVOT_RE.match(part))
            tail = "/".join(parts[idx + 1 :])
        except StopIteration:
            # Fallback: no known pivot found — use <site_dir>/<file>.
...
for r in rows:
    ...
    normal_post = resolve_path(r["post_accept_screenshot_path"]) if r["post_accept_screenshot_path"] else None
    normal_reject = resolve_path(r["post_reject_screenshot_path"]) if r["post_reject_screenshot_path"] else None
    lh_pre = lh_screenshot_path(r["pre_lh_path"])
    lh_post = lh_screenshot_path(r["post_accept_lh_path"])
    lh_reject = lh_screenshot_path(r["post_reject_lh_path"])

    # Build bbox overlay for normal pre screenshot when coordinates are available.
    bbox_overlay = ""
    if normal_pre and normal_pre.exists():
        x, y, w, h = (r["cookie_bbox_x"], r["cookie_bbox_y"], r["cookie_bbox_width"], r["cookie_bbox_height"])
    ...
    normal_reject_block = render_image_block(normal_reject)
    lh_pre_block = render_image_block(lh_pre)
    lh_post_block = render_image_block(lh_post)
    lh_reject_block = render_image_block(lh_reject)
    ...
    img_block = (
        ...
        "</div>"
        "<div>"
        '<div style="font-size:11px;color:#666;margin-bottom:3px">Post-accept</div>'
        f"{normal_post_block}"
        "</div>"
        "<div>"
        '<div style="font-size:11px;color:#666;margin-bottom:3px">Post-reject</div>'
        f"{normal_reject_block}"
        "</div>"
        "</div>"
        "</div>"
    ...
    audit_block = (
        ...
        f'<div style="font-size:12px;color:#444;margin:2px 0">'
        f"Lighthouse: pre={fmt_score(r['pre_lh_score'])}, "
        f"post-accept={fmt_score(r['post_accept_lh_score'])}, "
        f"post-reject={fmt_score(r['post_reject_lh_score'])}"
        f"</div>"
        f"{wave_triplet('WAVE pre', r, 'pre')}"
        f"{wave_triplet('WAVE post-accept', r, 'post_accept')}"
        f"{wave_triplet('WAVE post-reject', r, 'post_reject')}"
        "</div>"
    )
        ...
        notice_html = "".join(
            [
                ...
                badge_eff(
                    ...
                    "cookie_control_type",
                    "manual_cookie_control_type",
                    ("accept_or_reject", "accept_reject_or_settings", "accept_only", "accept_or_settings"),
                ),
    ...
    # Acceptance/rejection status badges
    scan_id = r["id"]
    accepted = r["cookie_notice_accepted"]
    attempted = r["cookie_accept_attempted"]
    rejected = r["cookie_notice_rejected"]
    reject_attempted = r["cookie_reject_attempted"]
    ...
    if fp:
        ...
        accept_status = (
            '<span style="background:#e76f51;color:#fff;padding:2px 8px;'
            'border-radius:3px;font-size:12px">not accepted (manually verified)</span>'
        )
    elif attempted:
        accept_status = (
            '<span style="background:#e9c46a;color:#333;padding:2px 8px;'
    ...
    # Reject status badge (shown when the reject phase ran)
    if reject_attempted and not fp:
        if rejected:
            reject_status = (
                '<span style="background:#457b9d;color:#fff;padding:2px 8px;'
                'border-radius:3px;font-size:12px;margin-left:6px">rejected</span>'
            )
        else:
            reject_status = (
                '<span style="background:#e9c46a;color:#333;padding:2px 8px;'
                'border-radius:3px;font-size:12px;margin-left:6px">reject attempted — not confirmed</span>'
            )
    else:
        reject_status = ""

    reject_verify_btn = ""
    if reject_attempted and not rejected and not fp:
        reject_verify_btn = (
            f'<div style="margin-top:6px" class="reject-verify-controls" data-scan-id="{scan_id}">'
            f'<span style="font-size:12px;color:#555;margin-right:6px">Manual reject verify:</span>'
            f'<button onclick="setRejectVerified({scan_id},1)" style="font-size:12px;padding:2px 8px;'
            f'margin-right:4px;cursor:pointer;background:#457b9d;color:#fff;border:none;border-radius:3px">'
            f"Mark rejected</button>"
            f'<button onclick="setRejectVerified({scan_id},0)" style="font-size:12px;padding:2px 8px;'
            f'cursor:pointer;background:#e76f51;color:#fff;border:none;border-radius:3px">'
            f"Mark not rejected</button>"
            f'<button onclick="setRejectVerified({scan_id},null)" style="font-size:12px;padding:2px 8px;'
    ...
    # returned no useful result (position NULL or 'none', emphasis 'none') OR manual overrides exist.
    # Never show on false positives — there's nothing to classify.
    needs_classify = (
        r["cookie_notice_detected"]
        and not fp
        and (
            CONTROL_TYPE_FILTER is not None
            or eff_position is None
            or eff_position == "none"
            or eff_emphasis is None
            or eff_emphasis == "none"
            or r["manual_cookie_position"] is not None
            or r["manual_cookie_emphasized_option"] is not None
        )
    )
    classify_panel = ""
    if needs_classify:
        ...
        def _sel(fid, options, current):
            ...
            return f'<select id="cls-{fid}-{scan_id}" style="font-size:11px;padding:1px 3px">{opts}</select>'
        ...
        classify_panel = (
            ...
            f"</div>"
            f'<div style="margin-top:6px">'
            f'<button onclick="setAllClassifications({scan_id})" '
            f'style="font-size:12px;padding:3px 10px;cursor:pointer;'
            f'background:#555;color:#fff;border:none;border-radius:3px">Set all</button>'
            f"</div>"
            f"</div>"
        )
    ...
    <div style="border:1px solid #ddd;border-radius:6px;padding:12px;background:#fafafa">
      ...
      <div style="margin-bottom:6px">{accept_status}{reject_status}</div>
      {img_block}
            {audit_block}
...
<body>
  ...
  <p>
    ...
    {sum(1 for r in rows if r["cookie_accept_attempted"] and not r["cookie_notice_accepted"] and not r["false_positive"])} accept-attempted but unconfirmed
    (yellow = awaiting manual verification).
    {sum(1 for r in rows if r["cookie_notice_rejected"] and not r["false_positive"])} successfully rejected
    (blue badge) |
    {sum(1 for r in rows if r["cookie_reject_attempted"] and not r["cookie_notice_rejected"] and not r["false_positive"])} reject-attempted but not confirmed
    (yellow badge).
    {sum(1 for r in rows if r["cookie_notice_detected"] and not r["false_positive"] and ((not r["cookie_position"] or r["cookie_position"] == "none") and not r["manual_cookie_position"]) or ((not r["cookie_emphasized_option"] or r["cookie_emphasized_option"] == "none") and not r["manual_cookie_emphasized_option"] and r["cookie_notice_detected"] and not r["false_positive"]))} unclassified
    ...
    <div class="controls">
        ...
        &nbsp;&nbsp;
        <label>
            <input type="checkbox" id="toggle-only-unclassified">
  ...
  <div class="grid">{"".join(cards)}</div>
    <script>
        ...
        const toggleOnlyUnclassified = document.getElementById("toggle-only-unclassified");
        if (toggleOnlyUnclassified) {{
            toggleOnlyUnclassified.addEventListener("change", (event) => {{
                const onlyUnclassified = event.target.checked;
                for (const card of document.querySelectorAll(".grid > div")) {{
                    const needsClassify = !!card.querySelector(".classify-controls");
                    if (onlyUnclassified) {{
                        card.style.display = needsClassify ? "" : "none";
                    }} else {{
                        card.style.display = "";
                    }}
                }}
                if (onlyUnclassified && toggleUnclassified) {{
                    toggleUnclassified.checked = true;
                }}
            }});
        }}

        function setRejectVerified(scanId, value) {{
            const val = value === null ? "NULL" : value;
            const cmd = `python mark_reject_verified.py "${{DB_PATH}}" ${{scanId}} ${{val}}`;
        ...
        function setAllClassifications(scanId) {{
            const fields = ['position', 'control_type', 'emphasized_option', 'has_reject', 'has_settings', 'pre_selected'];
            const parts = [];
            for (const f of fields) {{
                const el = document.getElementById(`cls-${{f}}-${{scanId}}`);
                if (el && el.value) {{ parts.push(f, el.value); }}
            }}
            if (parts.length === 0) {{ alert("Please select values first."); return; }}
            const cmd = `python mark_classified.py "${{DB_PATH}}" ${{scanId}} ${{parts.join(' ')}}`;
            navigator.clipboard.writeText(cmd).then(() => {{
                alert("Copied to clipboard — paste and run in your terminal:\\n\\n" + cmd);
            }}).catch(() => {{
                prompt("Run this command in your terminal:", cmd);
